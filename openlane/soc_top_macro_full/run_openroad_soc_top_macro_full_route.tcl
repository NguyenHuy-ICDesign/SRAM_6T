puts "========================================"
puts " OpenROAD Full-Backend v4 - soc_top_macro"
puts " up to Detailed Route"
puts "========================================"
...
proc obj_to_name {obj} {
    if {![catch {set n [sta::get_full_name $obj]}]} {
        return $n
    }
    if {![catch {set n [get_name $obj]}]} {
        return $n
    }
    return $obj
}
...
initialize_floorplan \
  -die_area  {0 0 1100 750} \
  -core_area {60 60 1040 690} \
  -site unithd
...
foreach h $macro_handles {
    lappend macro_insts [obj_to_name $h]
}
...
puts "\nRunning post-CTS detailed placement..."
detailed_placement
...
detailed_route \
  -output_drc results/${top_module}_dr_drc.rpt \
  -output_maze results/${top_module}_dr_maze.log \
  -drc_report_iter_step 5 \
  -droute_end_iter 20 \
  -clean_patches

write_def     results/${top_module}_full_dr.def
write_verilog results/${top_module}_full_dr.v
write_db      results/${top_module}_full_dr.odb

puts "✓ Saved: results/${top_module}_full_dr.def"
puts "✓ Saved: results/${top_module}_full_dr.v"
puts "✓ Saved: results/${top_module}_full_dr.odb"
...
puts "\n========================================"
puts "soc_top_macro full-backend v4 completed to detailed route."
puts "========================================"
